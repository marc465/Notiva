...
class FavouritesSearchResults extends StatefulWidget {
  final FlutterSecureStorage secureStorage;
  final SearchProviderFavourites searchProvFav;

  const FavouritesSearchResults({super.key, required this.secureStorage, required this.searchProvFav});

  @override
  State<FavouritesSearchResults> createState() => _FavouritesSearchResultsState();
}


class _FavouritesSearchResultsState extends State<FavouritesSearchResults> {
        ...
        dismissible: DismissiblePane(
          onDismissed: () async {
            await requestProvider.handleRequest(() async {
              return await widget.searchProvFav.deleteNote(note.getId);
            }, context);
          }
        ),
        children: [
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.searchProvFav.isLoading) {
      print("is loading");
      return const Center(
    ...
    if (widget.searchProvFav.result.isEmpty) {
      print("nothing found");
      return const Center(
    ...
    return Scaffold(
      body: ListView.builder(
        itemCount: widget.searchProvFav.result.length,
        itemBuilder: (context, index){
          return searchedNotesGenerator(context, widget.searchProvFav.result[index]);
        }
      )
    );
  }
