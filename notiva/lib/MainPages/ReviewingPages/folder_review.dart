...
class _FolderReviewState extends State<FolderReview> {
  ...
  void initialization() async {
    ...
    await requestProvider.handleRequest(
      () async {
        ...
        return provider.getInitialData(widget.folderId);
      }, 
      context);
    
    nameController.text = provider.collection.folderName;
    iconController.text = provider.collection.icon;
    descriptionController.text = provider.collection.folderDescription;

    nameController.addListener((){
      final text = nameController.text.trim();
      if (text == provider.collection.folderName) {
    ...
    descriptionController.addListener((){
      ...
      if (text == provider.collection.folderDescription) {
      provider.updateFieldStatus('description', false);
        return;
      }
      newDescription = text;
      provider.updateFieldStatus('description', true);
    });
    ...
    iconController.addListener(() {
      ...
      if (text == provider.collection.icon) {
        provider.updateFieldStatus('icon', false);
      } else {
        newIcon = text;
        provider.updateFieldStatus('icon', true);
      }
    });
  }
  ...
  Future<int> updateFolder() async {
    if (provider.changes.values.any((element) => element == true)) {
      
      final body = jsonEncode({
          if (provider.changes['name'] == true) 'folderName': newName,
          if (provider.changes['description'] == true) 'folderDescription': newDescription,
          if (provider.changes['icon'] == true) 'icon': newIcon,
          if (provider.changes['list'] == true) 'idOfNotesInFolder': provider
              .notesInCollection
              .map((note) => note.getId)
              .toList(),
        });
      ...
      final response = await http.post(
        Uri.parse("http://localhost:8080/folders/update"),
        headers: <String, String>{
          'Content-Type': 'application/json',
          'access_token': await widget.secureStorage.read(key: 'access_token') ?? "",
          'folder_id': provider.collectionId.toString()
        },
        body: body,
      );
      ...
      if (response.statusCode == 200) {
        ...
        provider.update(name: newName, description: newDescription, icon: newIcon);
        ...
        appBar: AppBar(
          ...
          actions: [
            CupertinoButton(
              ...
              onPressed: () async {
                await requestProvider.handleRequest(
                  () async {return await updateFolder();}, 
                  context
                );
              },
              child: Text(
                    ...
                    Padding(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: "Search in notes inside forlder",
                  ...
                  delegate: SliverChildBuilderDelegate(
                    (context, index) {
                      ...
                      if (provider.isCollectionSearchActive) {
                        note = provider.initialNotesCollection[index];
                      } else {
